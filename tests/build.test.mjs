...
test('build.mjs creates empty subdirectories inside dist/src even when they contain no files', async () => {
  const dir = await createTempProject();
  try {
    await writeFixtureFiles(dir);
    ...
    const info = await stat(join(dir, 'dist', 'src', 'empty-assets'));
...
test('build.mjs prints no output on stderr on a successful run', async () => {
  const dir = await createTempProject();
  try {
    await writeFixtureFiles(dir);
    const result = runBuild(dir);
